...
import { colorsId } from '../data/colors_id'
...
async function generateScoreCard(streak, reverse) {
  ...
  ctx.fillText('🇮🇩 Nama Warna Indonesia', PAD, y + 44); y += 80
  ctx.font = '28px system-ui, sans-serif'; ctx.fillStyle = '#94a3b8'
  ctx.fillText(reverse ? 'Warna → Nama  ·  Tanpa Batas' : 'Nama → Warna  ·  Tanpa Batas', PAD, y + 28); y += 70
  ...
  ctx.fillText('benar berturut-turut', SIZE / 2, y); y += 80
...
function loadStats() {
  try {
    const s = localStorage.getItem('colorguesser_namequiz_id_infinite_stats')
...
function saveStats(stats) {
  localStorage.setItem('colorguesser_namequiz_id_infinite_stats', JSON.stringify(stats))
}

function buildRound(usedIndices) {
  const available = [...colorsId.keys()].filter(i => !usedIndices.has(i))
  const pool = available.length >= 1 ? available : [...colorsId.keys()]
  const correctIdx = pool[Math.floor(Math.random() * pool.length)]
  const correct = colorsId[correctIdx]
...
export default function ColorNameQuizIdInfinite({ reverse = false }) {
  ...
  async function handleShareImage() {
    ...
    canvas.toBlob(async blob => {
      const file = new File([blob], 'nama-warna-indonesia-streak.png', { type: 'image/png' })
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        try { await navigator.share({ files: [file], title: 'Nama Warna Indonesia' }); setShareStatus(null); return }
        catch (e) { if (e.name === 'AbortError') { setShareStatus(null); return } }
      }
      const a = document.createElement('a'); a.href = URL.createObjectURL(blob)
      a.download = 'nama-warna-indonesia-streak.png'; a.click()
      setShareStatus('downloaded'); setTimeout(() => setShareStatus(null), 3000)
    })
  }

  async function handleShareText() {
    const mode = reverse ? 'Warna→Nama' : 'Nama→Warna'
    const text = [
      `🇮🇩 Nama Warna Indonesia — ${mode} Tanpa Batas`,
    ...
    return (
      <div className="min-h-screen bg-paper font-body flex flex-col">
        <header className="border-b border-ink/10 bg-canvas px-4 py-3 flex items-center gap-3">
          <a href={reverse ? '#/colorguesser/idswatchquiz' : '#/colorguesser/idquiz'} className="text-accent hover:underline text-sm font-medium">← Back</a>
          <h1 className="font-display text-xl font-bold text-ink flex-1 text-center">
            ♾️ {reverse ? 'Warna → Nama' : 'Nama → Warna'}
          </h1>
          <div className="w-12" />
        </header>
        <div className="flex-1 flex flex-col items-center justify-center px-4 py-10 max-w-md mx-auto w-full gap-6">
          <div className="text-center">
            ...
            <div className="text-ink/50 mt-1">benar berturut-turut</div>
            {isNewBest && streak > 0 && (
              <div className="mt-2 text-sm font-bold text-green-600 animate-pulse">🎉 Rekor baru!</div>
            )}
            {!isNewBest && (
              <div className="mt-2 text-sm text-ink/40">Terbaik: {stats.bestStreak}</div>
            )}
          </div>
          ...
          {wrongColor && (
            <div className="bg-white border border-red-100 rounded-xl p-4 flex items-center gap-3 w-full">
              ...
              />
              <div>
                <div className="text-xs text-red-400 font-semibold uppercase tracking-wide mb-0.5">Kamu salah</div>
          ...
          {/* Share buttons */}
          <div className="text-xs font-semibold text-ink/40 uppercase tracking-wider text-center">Bagikan hasilmu</div>
          <div className="flex gap-2 w-full">
            ...
            >
              {shareStatus === 'generating' ? '⏳' : shareStatus === 'downloaded' ? '✓ Tersimpan!' : '📸 Gambar'}
            </button>
            <button
            ...
            >
              {shareStatus === 'copied' ? '✓ Disalin!' : '📋 Teks'}
            </button>
          </div>
          ...
          <div className="flex flex-col gap-3 w-full">
            ...
            <a
              href={reverse ? '#/colorguesser/idswatchquiz' : '#/colorguesser/idquiz'}
      ...
      <div className="flex-1 overflow-auto px-4 py-6 max-w-xl mx-auto w-full flex flex-col gap-6">
        ...
        <div className="text-center">
          ...
          <div className="text-ink/50 text-sm mt-3">
            {reverse ? 'Apa nama warna ini?' : 'Mana warna yang sesuai dengan nama ini?'}
          </div>
        </div>
            ...
            {round.options.map((opt, idx) => {
              ...
              return (
                <button key={idx} onClick={() => handlePick(idx)} disabled={phase !== 'playing'}
                  ...
                  aria-label={phase === 'result' ? opt.name : `Pilihan warna ${idx + 1}`}
                >
                  {overlay}
                </button>
              )
        ...
        {phase === 'result' && (
          <div className="bg-white border border-ink/10 rounded-xl p-4 flex flex-col gap-3">
            ...
            >
              {lastIsCorrect ? `Selanjutnya → (streak: ${streak + 1})` : 'Lihat hasil'}
            </button>
          </div>
        )}
      </div>
