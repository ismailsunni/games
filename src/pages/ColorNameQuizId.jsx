...
function getRating(score) {
  ...
  if (pct === 1) return { label: 'Sempurna!', emoji: '🏆' }
  if (pct >= 0.8) return { label: 'Luar Biasa', emoji: '🥇' }
  if (pct >= 0.6) return { label: 'Bagus', emoji: '🥈' }
  if (pct >= 0.4) return { label: 'Lumayan', emoji: '🥉' }
  return { label: 'Terus Berlatih', emoji: '🎨' }
}

function loadStats() {
  try {
    const s = localStorage.getItem('colorguesser_namequiz_id_stats')
...
function saveStats(stats) {
  localStorage.setItem('colorguesser_namequiz_id_stats', JSON.stringify(stats))
}

function buildRounds(count) {
  const indices = [...colorsId.keys()].sort(() => Math.random() - 0.5).slice(0, count)
...
export default function ColorNameQuizId({ reverse = false }) {
  ...
  function handlePlayAgain() {
    window.location.hash = reverse ? '#/colorguesser/idswatchquiz/play' : '#/colorguesser/idquiz/play'
      ...
      <div className="min-h-screen bg-paper font-body flex flex-col">
        <header className="border-b border-ink/10 bg-canvas px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="font-display text-xl font-bold text-ink flex-1 text-center">
            {reverse ? '🎨 Warna → Nama' : '🏷️ Nama → Warna'}
          </h1>
        ...
        <div className="flex-1 overflow-auto px-4 py-6 max-w-xl mx-auto w-full">
          <div className="text-center mb-6">
            ...
            <div className="text-ink/60 text-sm mt-1">
              dari {(TOTAL_ROUNDS * POINTS_PER_ROUND).toLocaleString()} · {rating.label}
            </div>
          ...
          <div className="bg-white border border-ink/10 rounded-lg overflow-hidden mb-6">
            <div className="px-4 py-2 bg-canvas border-b border-ink/10">
              <span className="text-xs font-semibold text-ink/60 uppercase tracking-wide">Hasil</span>
            </div>
      ...
      <div className="flex-1 overflow-auto px-4 py-6 max-w-xl mx-auto w-full flex flex-col gap-6">
        ...
        {phase === 'result' && (
          <div className="bg-white border border-ink/10 rounded-xl p-4 flex flex-col gap-3">
            ...
            >
              {currentRound + 1 >= TOTAL_ROUNDS ? 'Lihat hasil →' : 'Selanjutnya →'}
            </button>
          </div>
        )}
        ...
        <div className="text-center text-sm text-ink/50">
          Skor: {totalScore.toLocaleString()} / {((currentRound + (phase === 'result' ? 1 : 0)) * POINTS_PER_ROUND).toLocaleString()}
        </div>
      </div>
    </div>
  )
